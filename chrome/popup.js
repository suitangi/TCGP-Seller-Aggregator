...
function getCards() {
  chrome.runtime.sendMessage({ msgType: 'getCards' }).then((result) => {
    cards = result || [];
    console.log('Cards in cart:', cards);
    for(c of cards) {
...
function updateCardQuantity(cardId, isFoil, quantity) {
  ...
  if (card) {
    ...
    if (quantity === 0) {
      chrome.runtime.sendMessage({ msgType: 'removeCard', id: cardId, isFoil: isFoil }).then(() => {
...
async function removeCard() {
  ...
  await chrome.runtime.sendMessage({ msgType: 'removeCard', id: id, isFoil: isFoil });
  cards = cards.filter(c => !(c.id === id && c.isFoil === isFoil));
  // Re-render cart to ensure consistent state - this removes the row immediately
  cartTable.innerHTML = '';
  getCards();
}
...
function aggregate3(_sellers) {
  ...
  for(s of sellerTotals) { //render loop. sellers
    ...
    for(card of cards) { //loop cards per seller
      ...
      // Removed inCart seller counting
    ...
    // Calculate shipping cost from the first available listing for this seller
    let shippingCost = 0;
    let sellerShippingPrice = 0;
    for(card of cards) {
      if(card.inCart) continue;
...
function solveSetCover(availableCards, sellers) {
  ...
  for (const request of cardRequests) {
    ...
    for (let sellerIdx = 0; sellerIdx < cardSellers.length; sellerIdx++) {
      ...
      if (seller && seller.quantity > 0) {
        // Debug: Log seller mapping
        console.log(`  Seller ${seller.sellerId}: originalSellerIdx=${request.sellerIdx}, originalSellerIdxIdx=${sellerIdx}`);
...
if (optimizedBtn) {
  optimizedBtn.addEventListener("click", optimizeCart);
}